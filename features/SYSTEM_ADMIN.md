...
6. **Cross-install secret rewrap.** Walks every Fernet-encrypted column in the schema plus the `__enc__:`-prefixed fields inside `backup_target.config` JSONB; decrypts each with the source key recovered from `secrets.enc`, re-encrypts with the destination's local key, UPDATEs in place. Same-install restores short-circuit with `same_install=true`. The operator does not have to copy the recovered `SECRET_KEY` into the destination's `.env` manually.

   The column list in `app/services/backup/rewrap.py` is deliberately explicit rather than auto-discovered, so that adding an encrypted column is a review decision — and `tests/test_rewrap_coverage.py` asserts it matches the mapped metadata exactly. That guard exists because the list had silently drifted to 21 of 47 columns (#781): everything it missed stayed encrypted under the *source* install's key, which surfaces only on first use of the affected credential. On the appliance this is the default case, not an edge case — `spatiumddi-firstboot` mints a fresh `SECRET_KEY` on every install, so every restore onto reimaged hardware is a cross-install restore.
